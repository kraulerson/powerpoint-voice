...
def tiny_jpeg():
    """A minimal baseline JPEG, 8x8 solid red, built here so the fixtures need no
    external tooling. Every other image fixture is a PNG, which is why deleting JPEG
    support entirely left the whole suite green (BUG-61) — while the reference deck
    contains two JPEG media parts."""
    import struct, zlib  # noqa: F401
    # Quantisation table (all 16 -> coarse but valid), one luma + one chroma.
    qt = bytes([16] * 64)
    # Baseline DC/AC Huffman tables from the JPEG spec's example (Annex K).
    ...
    out = b"\xff\xd8"                                     # SOI
    out += seg(0xDB, b"\x00" + qt) + seg(0xDB, b"\x01" + qt)
    out += seg(0xC0, struct.pack(">BHHB", 8, 8, 8, 3)
               + bytes([1, 0x11, 0, 2, 0x11, 1, 3, 0x11, 1]))   # SOF0 8x8, 3 comp
    out += seg(0xC4, b"\x00" + dc_bits + dc_vals) + seg(0xC4, b"\x10" + ac_bits + ac_vals)
    out += seg(0xC4, b"\x01" + dc_bits + dc_vals) + seg(0xC4, b"\x11" + ac_bits + ac_vals)
    out += seg(0xDA, bytes([3, 1, 0x00, 2, 0x11, 3, 0x11, 0, 63, 0]))  # SOS
    # One MCU of solid red: Y=82 Cb=90 Cr=240 as DC-only coefficients.
    out += bytes([0x54, 0x8B, 0x1F, 0xE0, 0x7F, 0xFF, 0xD9])
    return out


def build_good_jpeg_image():
    # BUG-61: JPEG decode had NO fixture, so removing JPEG from the allow-list left
    # the suite green while the reference deck's two JPEGs would render as grey
    # placeholder boxes.
    parts = {
        "[Content_Types].xml": content_types(1, has_png=True),
        "_rels/.rels": root_rels(),
        "ppt/presentation.xml": presentation_xml(1),
        "ppt/_rels/presentation.xml.rels": presentation_rels(1),
        "ppt/slides/slide1.xml": slide_xml([pic_sp("rId1", 1000000, 1000000, 6000000, 4000000)]),
        "ppt/slides/_rels/slide1.xml.rels": slide_rels(image_target="../media/image1.jpeg"),
        "ppt/media/image1.jpeg": tiny_jpeg(),
    }
    write_zip("good_jpeg_image.pptx", parts)


def build_good_srcrect_negative():
    # BUG-62: negative insets and a right-edge sub-pixel crop. 6 of the reference
    # deck's 12 srcRect elements carry negative insets, and neither clamp had a test.
    parts = {
        "[Content_Types].xml": content_types(1, has_png=True),
        "_rels/.rels": root_rels(),
        "ppt/presentation.xml": presentation_xml(1),
        "ppt/_rels/presentation.xml.rels": presentation_rels(1),
        "ppt/slides/slide1.xml": slide_xml([
            pic_sp_cropped("rId1", 0, 0, 4000000, 3000000, r=-6769, b=41679),
            pic_sp_cropped("rId1", 4000000, 0, 4000000, 3000000, l=-6726, r=-1, b=36909),
            pic_sp_cropped("rId1", 8000000, 0, 4000000, 3000000, l=93750),
        ]),
        "ppt/slides/_rels/slide1.xml.rels": slide_rels(image_target="../media/image1.png"),
        "ppt/media/image1.png": wide_red_png(),
    }
    write_zip("good_srcrect_negative.pptx", parts)
...
if __name__ == "__main__":
    ...
    build_good_stretch()
    build_good_jpeg_image()
    build_good_srcrect_negative()
